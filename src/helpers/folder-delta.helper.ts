 ...
 * state. See the module docs for the full status grid and the resolution
 ...
 */
export function compareAt(snapshot: FileSnapshot | null | undefined, timestamp: number): FolderDeltaResult {
  if (!snapshot) {
    return { status: FolderDeltaStatus.none, base: [], current: [] };
  }

  const existed: boolean = existedAtT(snapshot, timestamp);
  ...
  const base: string[] = resolveBaseAt(snapshot, timestamp);
   ...
   */
  return {
    status: contentEquals(base, current) ? FolderDeltaStatus.none : FolderDeltaStatus.modified,
    base,
    current,
  };
}
 ...
 */
function existedAtT(snapshot: FileSnapshot, timestamp: number): boolean {
 ...
 */
function firstSeenAt(snapshot: FileSnapshot): number {
 ...
 */
function resolveBaseAt(snapshot: FileSnapshot, timestamp: number): string[] {
  const versions: readonly FileVersion[] = snapshot.timeline.getStoredVersions();
