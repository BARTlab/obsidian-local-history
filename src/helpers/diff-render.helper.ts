 ...
 */
function buildCleanPatch(params: DiffRenderParams): string {
 ...
 */
function buildDiff2HtmlInput(params: DiffRenderParams): string {
  const base: string = params.baseLines.join(params.lineBreak);
  const current: string = params.currentLines.join(params.lineBreak);
 ...
 * @param {DiffRenderParams} params - The render parameters
 */
function renderPatch(params: DiffRenderParams): void {
  const patch: string = buildCleanPatch(params);
 ...
 * @param {DiffRenderParams} params - The render parameters
 */
function renderInline(params: DiffRenderParams): void {
  const base: string = params.baseLines.join(params.lineBreak);
  const current: string = params.currentLines.join(params.lineBreak);
  ...
  diffLines.forEach((line: InlineDiffLine): void => {
    if (line.type === WordDiffLineType.context) {
      rows.push(makeInlineRow('context', ' ', [{ tag: 'span', text: line.oldText ?? '' }]));
    ...
    if (line.type === WordDiffLineType.added) {
      rows.push(makeInlineRow('added', '+', [{ tag: 'span', text: line.newText ?? '' }]));

      return;
    }

    if (line.type === WordDiffLineType.removed) {
      rows.push(makeInlineRow('removed', '-', [{ tag: 'span', text: line.oldText ?? '' }]));
    ...
    rows.push(makeInlineRow(
      'modified',
      '~',
      makeInlineWordSpans(line.oldText ?? '', line.newText ?? ''),
    ));
  });
 ...
 */
function renderDiff2Html(params: DiffRenderParams, format: DiffOutputFormatType): void {
  const diffHtml: string = Diff2Html.html(buildDiff2HtmlInput(params), {
    drawFileList: false,
    matching: 'lines',
    outputFormat: format,
 ...
 */
function makeInlineRow(kind: string, sign: string, content: DomElementConfig[]): DomElementConfig {
  return {
    tag: 'div',
 ...
 */
function makeInlineWordSpans(oldText: string, newText: string): DomElementConfig[] {
