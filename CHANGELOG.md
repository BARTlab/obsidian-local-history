...
- **Vault changes panel.** A right-sidebar panel lists every file that still differs from its history origin across the whole vault: modified files, files added under tracking, and deleted files (kept as tombstones). Toggle between a nested folder tree and a flat list that shows each file's path inline, filter by name, and click a file to open it. The panel docks into the right sidebar on first load; reopen it any time from the "Open vault changes panel" command.
