...
/**
 * Helpers for line-level diff hunks and single-hunk reverts.
 *
 ...
 * current text intact. All functions are pure and operate on arrays of lines, so
 ...
 */
export function baseLinesForHunk(hunk: Diff.StructuredPatchHunk): string[] {
  return (hunk?.lines ?? [])
 ...
 */
export function hunkAtLine(hunks: Diff.StructuredPatchHunk[], line: number): Diff.StructuredPatchHunk | null {
 ...
 */
export function revertHunk(currentLines: string[], hunk: Diff.StructuredPatchHunk): string[] {
  ...
  lines.splice(start, Math.max(0, hunk.newLines), ...baseLinesForHunk(hunk));
 ...
 */
export function revertDescriptor(currentLines: string[], hunk: Diff.StructuredPatchHunk): EditorBlock {
 ...
 */
export function classifyLine(line: string): HunkLineKind {
  if (line.startsWith('+')) {
    return 'added';
  }
