 ...
 */
export function match(versions: SelectableVersion[], baselineLines: string[], selection: string): Set<string> {
  ...
  const selectionLines: string[] = toSelectionLines(needle);
  ...
  for (const version of list) {
    ...
    if (
      allLinesPresent(selectionLines, added)
      || allLinesPresent(selectionLines, removed)
    ) {
 ...
 */
function collectChangedLines(previous: string[], current: string[]): { added: string[]; removed: string[] } {
  const added: string[] = [];
 ...
 */
function allLinesPresent(selectionLines: string[], changedLines: string[]): boolean {
